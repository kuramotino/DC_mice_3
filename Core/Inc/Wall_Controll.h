...
namespace controll
{
	class Wall_Ctrl : public BaseCtrl
	{
		...
		float THRESHOLD_R=50;//1右壁の閾値
		float THRESHOLD_DIFF_R=5;//1右壁の変化量の閾値
		float THRESHOLD_L=100;//2左壁の閾値
		float THRESHOLD_DIFF_L=5;//2左壁の変化量の閾値
		float SENSOR_GAIN=0.0033;//1壁制御のゲイン
		float CENTER_R=180;//機体が中心にあるときの右AD値
		float CENTER_L=250;//機体が中心にあるときの左AD値
